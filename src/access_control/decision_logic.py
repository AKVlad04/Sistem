...
ACCESS_POLICIES = {
    'Autoturism': {'Access': True, 'Fee_RON': 10.00, 'Zone': 'P1/P2', 'Notes': 'Taxa standard'},
    ...
    'Utilitara': {'Access': True, 'Fee_RON': 20.00, 'Zone': 'Service', 'Notes': 'Taxa livrari'},

    # Dacă e respins, nu afișăm și nu logăm taxă / zonă / note suplimentare.
    'Camion': {'Access': False, 'Fee_RON': 0.00, 'Zone': '-', 'Notes': ''},
    'Autobuz': {'Access': True, 'Fee_RON': 20.00, 'Zone': 'C (Traseu)', 'Notes': 'Taxa Traseu'},

    # Politica pentru clasa nouă
    'Altele': {'Access': False, 'Fee_RON': 0.00, 'Zone': '-', 'Notes': ''}
}
...
def log_access_event(policy_result):
    ...
    log_data = [
        ...
        policy_result.get('Probability', '')
    ]

    try:
        with open(VEHICLE_LOG_FILE, 'a') as f:
            if not file_exists:
                f.write("Timestamp,Tip_Vehicul,Decizie,Taxa_Aplicata,Nota,Probability\n")
...
def get_policy_decision(predicted_class_index):
    ...
    policy = ACCESS_POLICIES.get(
        class_name,
        {'Access': False, 'Fee_RON': 0.00, 'Zone': '-', 'Notes': ''}
    )

    # Dacă e respins, normalizăm: fără taxă și fără informații suplimentare.
    if not policy.get('Access', False):
        policy['Fee_RON'] = 0.00
        policy['Zone'] = '-'
        policy['Notes'] = ''

    # Adăugăm numele clasei și decizia pentru logare
